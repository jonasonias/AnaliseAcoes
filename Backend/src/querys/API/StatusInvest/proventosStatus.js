...
// Função para atualizar a tabela dystatus
...
async function fetchWithRetry(url, retries = 5, delay = 3000) {
    try {
        ...
        if (status === 403) {
            console.error(`Erro 403 ao acessar ${url}: Acesso negado. Verifique as permissões ou autenticação.`);
            throw error;
        } else if ([429, 502].includes(status) && retries > 0) {
    ...
    try {
        ...
        for (const ticker of tickers) {
            ...
            try {
                ...
                const earnings = {};
                if (data.assetEarningsYearlyModels) {
                    data.assetEarningsYearlyModels.forEach(item => {
                        earnings[item.rank] = item.value;
                    });
                }

                // Atualizando a tabela dystatus

...
const NotaEficiencia = ({ ticker }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/acoes/nota/eficiencia/${ticker}`);
  ...
  return (
    <div>
      <h2 style={{ marginLeft: '30%' }}>Nota de Eficiência</h2>

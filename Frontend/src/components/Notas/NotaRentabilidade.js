...
const NotaRentabilidade = ({ ticker }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/acoes/nota/rentabilidade/${ticker}`);
  ...
  return (
    <div>
      <h2 style={{ marginLeft: '24%' }}>Nota de Rentabilidade</h2>
      <table style={{ width: '115%', borderCollapse: 'collapse', margin: '0 10px' }}>
        <thead>
          <tr>
...
export default NotaRentabilidade;

...
const PegRatio = ({ ticker }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/multiplos/peg_ratio/${ticker}`);
  ...
  return (
    <div>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          <tr>
            <td style={{ border: '1px solid black', padding: '8px' }}>Peg Ratio</td>
